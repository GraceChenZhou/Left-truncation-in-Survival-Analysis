...
The main reference is Geskus (2011). The original R function, `crprep`, is included in the R package `mstate`. However, there are some issues with the weight assignment in the original function. After consulting with Dr. Geskus, the bugs were fixed in the function. Since this update is not yet available in the `mstate` package, you will need to manually run the updated function provided below to obtain accurate results.
